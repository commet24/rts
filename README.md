...
Фоновые изображения 9 мая и логотип лежат в `src/assets`. Шрифт `RTRONE`
сейчас подключён из `public/fonts/rtrone.svg`; если появятся `woff2/woff`, лучше
заменить подключение в `src/styles/global.css` на современные форматы.
